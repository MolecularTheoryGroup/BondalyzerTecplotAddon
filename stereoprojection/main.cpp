...
#include "tecutiltools_bondalyzer_stereographic_Exports.h"
#include "stringformat.h"
#ifdef MSWIN
    #define EXPORT_INIT_TEC_ADDON EXPORTFROMADDON
#else
    #define EXPORT_INIT_TEC_ADDON extern "C" tecutiltools_bondalyzer_stereographic_stereographic_API
#endif
											  ...
											  char **ErrMsg)             /* OUT (only if returning FALSE) */
{
	 ...
	 * Example:
	 *
	 * $!EXTENDEDCOMMAND COMMANDPROCESSORID='Stereographic Visualization Add-On' COMMAND='MYCOMMAND'
	 */
#if 0
    EntIndex_t zone = -1;
    LgIndex_t node = -1;
    ...
    double Y;
    double Z;
    if ( sscanf(MacroCommandString, "StereographicProjection Zone=%ld,N=%ld", &zone, &node) == 2 )
	{
        if ( 1 <= zone && zone <= TecUtilDataSetGetNumVars() )
        {
            Mode = node;
            isOk = TRUE;
        }
	}
    else if (sscanf(MacroCommandString, "StereographicProjection Zone=%ld,X=%lf,Y=%lf,Z=%lf", &zone, &X, &Y, &Z) == 4)
    {
        if (1 <= zone && zone <= TecUtilDataSetGetNumVars())
        {
            Mode = XYZ;
    ...
    else if (sscanf(MacroCommandString, "StereographicProjection Zone=%ld,M=ALL", &zone, &X, &Y, &Z) == 4)
    {
        if (1 <= zone && zone <= TecUtilDataSetGetNumVars())
        {
            Mode = AllBonds;
        ...
        isOk = FALSE; // no macro language for now
#endif
	...
	if (!isOk)
	{
		...
		*ErrMsg = TecUtilStringAlloc(messageSize, "String for Error Message");
		snprintf(*ErrMsg, messageSize, "%s", "Error processing macro command");
	}
	else
	{
...
/**
 */
static void STDCALL stereoVisSinglePtMenuCB(void)
{
	TecUtilLockStart(AddOnID);
	stereoVis(StereoVisOp_SinglePt);
	TecUtilLockFinish(AddOnID);
}

/**
 */
static void STDCALL stereoVisAllCpsMenuCB(void)
{
	TecUtilLockStart(AddOnID);
	stereoVis(StereoVisOp_AllCps);
	TecUtilLockFinish(AddOnID);
}

/**
 */
static void STDCALL plotSingleZoneInNewFrameMenuCB(void)
{
	TecUtilLockStart(AddOnID);
	plotSingleZone();
	TecUtilLockFinish(AddOnID);
 ...
 */
EXPORT_INIT_TEC_ADDON void STDCALL InitTecAddOn(void)
{
	/*
    ...
    TecUtilMenuAddOption("Bondalyzer",
						 "Stereographic Visualization Single Point",
						 '\0',
						 stereoVisSinglePtMenuCB);
    TecUtilMenuAddOption("Bondalyzer",
                         "Stereographic Visualization All Critical Paths",
                         '\0',
                         stereoVisAllCpsMenuCB);
#if 0
    TecUtilMenuAddOption("Bondalyzer",
                         "Plot Single Zone in New Frame (for testing purposes)",
                         '\0',
                         plotSingleZoneInNewFrameMenuCB);
#endif
