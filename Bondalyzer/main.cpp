...
static void STDCALL MenuCallback(void)
{
    ...
    if (TecUtilDataSetIsAvailable())
    {
		...
		else
		{
			...
			ExtractTopology(1, RefinedGridSet, 5, 6, 7, 8, 4, CompletionLevel_ARCSurfaces, FALSE);
			ChemSysView();
		}
    }
    else
...
static void STDCALL ChemSysViewMenuCallback(void)
{
	TecUtilLockStart(AddOnID);
	if (TecUtilDataSetIsAvailable())
	{
...
static void STDCALL CalcVarsCallback(void)
{
	TecUtilLockStart(AddOnID);
	if (TecUtilDataSetIsAvailable())
	{
		CreateGradVectGradMagVars();
	}
	else
	{
		TecUtilDialogErrMsg("No data set in current frame.");
...
static void STDCALL SGradTopoViewMenuCallback(void)
{
	TecUtilLockStart(AddOnID);
	if (TecUtilDataSetIsAvailable())
	{
...
static void STDCALL CalcGradGradMagMenuCallback(void)
{
	TecUtilLockStart(AddOnID);
	if (TecUtilDataSetIsAvailable())
	{
		CalcGradGradMag(FALSE);
	}
	else
	{
		TecUtilDialogErrMsg("No data set in current frame.");
...
static void STDCALL CalcGradGradMagPeriodicMenuCallback(void)
{
	TecUtilLockStart(AddOnID);
	if (TecUtilDataSetIsAvailable())
	{
		CalcGradGradMag(TRUE);
	}
	else
	{
		TecUtilDialogErrMsg("No data set in current frame.");
