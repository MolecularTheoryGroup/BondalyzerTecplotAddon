...
using namespace tpcsm;

AddOn_pa AddOnID;

static char const* UPDATE_SPHERICAL_TRIANGULATION_COMMAND = "Op=UpdateSpherical";
static char const* CALCULATE_GRADIENT_ON_SPHERE_COMMAND = "Op=CaculateGradientsOnSphere";
...
// simple convenience function to return new or existing var number (1-based as TecUtil likes)
EntIndex_t getOrAddVariableByName(
    char const* varName,
    ValueLocation_e valueLocation = ValueLocation_Nodal,
    FieldDataType_e fieldDataType = FieldDataType_Float)
{
    EntIndex_t varNum = TecUtilVarGetNumByName(varName);
    if (varNum == TECUTILSETNOTMEMBER)
    {
        EntIndex_t const numZones = TecUtilDataSetGetNumZones();

        std::vector<ValueLocation_e> valueLocationArray;
        valueLocationArray.resize(numZones, valueLocation);
        std::vector<FieldDataType_e> fieldDataTypeArray;
        fieldDataTypeArray.resize(numZones, fieldDataType);
        ...
        TecUtilArgListAppendArray(argList, SV_VARDATATYPE, &fieldDataTypeArray[0]);
        VERIFY(TecUtilDataSetAddVarX(argList));
        TecUtilArgListDealloc(&argList);

        varNum = TecUtilVarGetNumByName(varName);

        Set varSet(varNum);
        CHECK(varNum != TECUTILSETNOTMEMBER);
        TecUtilStateChanged(StateChange_VarsAdded, ArbParam_t(varSet.getRef()));
    }
    return varNum;
}
...
/**
*/
char const* performCalculateGradientOnSphere(
    char const* dxName,
    char const* dyName,
    char const* dzName,
    char const* magName = NULL, // convenience, set to NULL if you don't want
    GradientCalcMethod_e gradientCalcMethod = GradientCalcMethod_AllConnectedCellCenters) // recommended
{
    using tpcsm::Vec3;
    ...
    if (TecUtilFrameGetPlotType() != PlotType_Cartesian3D)
        errMsg = "Calculate gradient on sphere test requires a 3-D plot.";

    Set_pa activeZones = NULL;
    if (errMsg == NULL && !TecUtilZoneGetActive(&activeZones))
        errMsg = "Calculate gradient on sphere test encountered memory error allocating active zone set.";
            ...
            if (TecUtilZoneIsEnabled(zone + 1) && TecUtilZoneIsActive(zone + 1))
            {
                    ...
                    (zoneType == ZoneType_Ordered && srcI > 1 && srcJ > 1 && srcK == 1))
                {
                    if (srcSphericalZoneNum == 0)
                        ...
                        errMsg = "Calculate gradient on sphere test detected multiple spherical surface zones, either FE-triangle or IJ-ordered.";
                }
                ...
                if (errMsg != NULL)
                {
                    ...
                    {
                        errMsg = "Calculate gradient on sphere test requires X, Y and Z variables be nodal for spherical surface.";
                    }
                }
            }
        }
    ...
    {
        if (srcSphericalZoneNum == 0)
            errMsg = "Calculate gradient on sphere test requires a single spherical surface zone, either FE-triangle or IJ-ordered.";
    }
    ...
    if (errMsg == NULL)
    {
        ...
        EntIndex_t zVarNum;
        TecUtilAxisGetVarAssignments(&xVarNum, &yVarNum, &zVarNum);
        EntIndex_t scalarVarNum = 4; // TODO: push back actual value

        StyleValue sv;
        VERIFY(StyleValue::returnCodeOk(sv.get(&scalarVarNum, 1, SV_GLOBALCONTOUR, SV_VAR)));
        if (scalarVarNum < 1)
            errMsg = "Contour group 1 must have value variable set";

        if (errMsg == NULL && !TecUtilStateIsProcessingMacro())
        {
            char message[2000];
            _snprintf(message, sizeof(message), "Calculate surface gradients for variable %d on spherical zone %d?",
                scalarVarNum,
                srcSphericalZoneNum);
            if (!TecUtilDialogMessageBox(message, MessageBoxType_YesNo))
                errMsg = "Spherical surface gradient calculation canceled.";
        }

        if (errMsg)
            return errMsg; // error out
        ...
        LgIndex_t numSurfaceTriangles = 0;
        std::vector<TriNodes> surfaceTriangles;
        std::vector<double>   scalarValues;

        if (zoneType == ZoneType_FETriangle)
        {
            numSurfaceTriangles = srcJ;
            surfaceTriangles.reserve(numSurfaceTriangles);
            NodeMap_pa NM = TecUtilDataNodeGetReadableRef(srcSphericalZoneNum);
            for (LgIndex_t elem = 0; elem < numSurfaceTriangles; elem++)
            {
                Index_t const n1 = TecUtilDataNodeGetByRef(NM, elem + 1, 1) - 1; // convert between 1-based TecUtil and 0-based std::vector
                ...
                surfaceTriangles.push_back(TriNodes(n1, n2, n3));
            }

            FieldData_pa scalarD = TecUtilDataValueGetReadableRef(srcSphericalZoneNum, scalarVarNum);
            if (TecUtilDataValueGetLocationByRef(scalarD) != ValueLocation_CellCentered)
                errMsg = "Contour variable is not cell centered";

            if (errMsg == NULL)
            {
                for (LgIndex_t elem = 0; elem < numSurfaceTriangles; elem++)
                {
                    double const scalarValue = TecUtilDataValueGetByRef(scalarD, elem + 1); // convert between 1-based TecUtil and 0-based std::vector
        ...
        {
            // future extension to IJ-ordered spheres would require stitching up zone edges that meet up around the sphere
            errMsg = "Calculate gradient on sphere test does not support IJ-zones";
        }
        CHECK(numSurfaceTriangles > 0);
        CHECK(surfaceTriangles.size() == size_t(numSurfaceTriangles));

        std::vector<Vec3> gradients;
        if (calculateGradientOnSphere(
                nodeXYZs, // in: nodal
                scalarValues, // in: cell-centered
                surfaceTriangles, // in: cell-based
                sphereCenter, // in: single XYZ coordinate
                sphereRadius, // in: single scalar value
                gradientCalcMethod, //in: gradientCalculationMethod
                gradients, // out: cell-centered dx,dy,dz
                errMsg)) // if an error
        {
            errMsg = NULL;

            EntIndex_t dxVarNum = getOrAddVariableByName(dxName, ValueLocation_CellCentered);
            EntIndex_t dyVarNum = getOrAddVariableByName(dyName, ValueLocation_CellCentered);
            EntIndex_t dzVarNum = getOrAddVariableByName(dzName, ValueLocation_CellCentered);
            EntIndex_t magVarNum = magName ? getOrAddVariableByName(magName, ValueLocation_CellCentered) : 0;

            FieldData_pa dxD = TecUtilDataValueGetWritableNativeRef(srcSphericalZoneNum, dxVarNum);
            FieldData_pa dyD = TecUtilDataValueGetWritableNativeRef(srcSphericalZoneNum, dyVarNum);
            FieldData_pa dzD = TecUtilDataValueGetWritableNativeRef(srcSphericalZoneNum, dzVarNum);
            FieldData_pa magD = magVarNum ? TecUtilDataValueGetWritableNativeRef(srcSphericalZoneNum, magVarNum) : NULL;
            CHECK(TecUtilDataValueGetLocationByRef(dxD) == ValueLocation_CellCentered);
            CHECK(TecUtilDataValueGetLocationByRef(dyD) == ValueLocation_CellCentered);
            CHECK(TecUtilDataValueGetLocationByRef(dzD) == ValueLocation_CellCentered);
            CHECK(magD ? TecUtilDataValueGetLocationByRef(magD) == ValueLocation_CellCentered : true);

            for (LgIndex_t cell = 1; cell <= numSurfaceTriangles; cell++)
            {
                TecUtilDataValueSetByRef(dxD, cell, gradients[cell - 1].x());
                TecUtilDataValueSetByRef(dyD, cell, gradients[cell - 1].y());
                TecUtilDataValueSetByRef(dzD, cell, gradients[cell - 1].z());
                if (magD)
                    TecUtilDataValueSetByRef(magD, cell, gradients[cell - 1].getNorm());
            }
            ...
            if ( magVarNum )
                varSet += magVarNum;
            TecUtilStateChanged(StateChange_VarsAltered, ArbParam_t(varSet.getRef()));
        }
    }
    ...
    Index_t& numCoincidentNodesIgnored)
{
            ...
            if (TecUtilZoneIsEnabled(zone+1) && TecUtilZoneIsActive(zone+1))
            {
                    ...
                    (zoneType == ZoneType_Ordered && srcI > 1 && srcJ > 1 && srcK == 1))
                {
                    if (srcSphericalZoneNum == 0)
                        ...
                        errMsg = "Update spherical triangulation detected multiple spherical surface zones, either FE-triangle or IJ-ordered.";
                }
    ...
    if (errMsg == NULL)
    {
                ...
                errMsg))
        {
            ...
            EntIndex_t const edgeVarNum = getOrAddVariableByName("Edge");
...
/**
*/
static void STDCALL calcGradientsOnSphere(void)
{
    TecUtilLockStart(AddOnID);

    char const* errMsg = performCalculateGradientOnSphere(
        "dx(lib)", "dy(lib)", "dz(lib)", "mag(lib)");
    if (errMsg)
        TecUtilDialogMessageBox(errMsg, MessageBox_Error);
    else
    {
        if (TecUtilMacroIsRecordingActive())
            TecUtilMacroRecordAddOnCommand(ADDON_NAME, CALCULATE_GRADIENT_ON_SPHERE_COMMAND);

        std::stringstream ss;
        ss << "Gradients calculated for sphere.";
    ...
    char **errMsgPtr)             /* OUT (only if returning FALSE) */
{
    ...
    if (stricmp(macroCommandString, CALCULATE_GRADIENT_ON_SPHERE_COMMAND) == 0)
    {
        errMsg = performCalculateGradientOnSphere("dx(lib)", "dy(lib)", "dz(lib)", "mag(lib)");
    }
    else if (stricmp(macroCommandString, UPDATE_SPHERICAL_TRIANGULATION_COMMAND) == 0)
    {
        Index_t numNodes = 0;
        Index_t numTriangles = 0;
        Index_t numCoincidentNodesIgnored = 0;
...
EXPORTFROMADDON void STDCALL InitTecAddOn(void)
{
    ...
    TecUtilMenuAddOption("Tools",
        "Calculate Gradients on Sphere",
        '\0',
        calcGradientsOnSphere);
